...
module.exports = {
	...
	async execute(interaction, client) {
		...
		if (!interaction.member.roles.cache.some(role => role.name === TeamName)) {
		...
		try {
			...
			// 	if (!TeamLogo.includes('http')) {
			// 		let emoji = interaction.guild.emojis.cache.find((emj) => emj.name === TeamLogo);
			// 	 console.log(emoji);
			// 		TeamLogoArray.push((interaction.guild.emojis.cache.find((emj) => emj.name === TeamLogo)).url);
			// 		console.log('Pushed in emoji URL');
			// 	};

			// 	TeamLogoArray.push(TeamLogo);

			// } else {
			// 	TeamLogoArray.push('https://cdn.discordapp.com/attachments/753693196680429698/891694506343284757/2EZ_animated_transparant.gif');
			// };
		...
		const DescriptionMessage = (`
			...
			<:coach:827176895196037192> Assistant Coach: ${CoachTwoArray.toString()}
